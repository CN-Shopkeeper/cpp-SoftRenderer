...
struct
{
    ...
    {Vec4{-160, -140, 0, 1}, Vec4{1, 0, 0, 1}},
    {Vec4{160, -140, 0, 1}, Vec4{0, 1, 0, 1}},
    {Vec4{0, 140, 0, 1}, Vec4{0, 0, 1, 1}},
};

constexpr int WindowWidth = 480;
constexpr int WindowHeight = 320;

const Mat44 OrthoMat = CreateOrtho(-300, 300, -300, 300, -1, 1);
...
public:
    ...
    void OnKeyDown(const SDL_KeyboardEvent &e) override
    {
        ...
        {
            scale_.z -= 1 / (scale_.x + 10);
        }
        if (e.keysym.sym == SDLK_n)
        {
            scale_.z += 1 / (scale_.x + 10);
        }
        ModelMat = CreateTranslate(pos_.x, pos_.y, 0) * CreateRotate(Radians(rot_.x), Radians(rot_.y), Radians(rot_.z)) * CreateScale(scale_.x, scale_.y, scale_.z);
    }
